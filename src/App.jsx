...
export default function App() {
  ...
  const loadReport = useCallback(async (id = merchantId) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/reports/${id}?period=${encodeURIComponent(period)}`, { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
      const data = await res.json();
      setReport(data);
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      try {
        const res = await fetch(`${API_URL}/api/crawl-jobs/${crawlJobId}`, { 
          headers: { "Authorization": `Bearer ${token}` } 
        });
  ...
  }, [crawlJobId, crawlStatus, token, loadReport, merchantId]);
  ...
  async function loadClaudeReport() {
    ...
    setActiveTab("claude");
    try {
      const res = await fetch(`${API_URL}/api/reports/${merchantId}/claude-report`, { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
  ...
  async function downloadExcel(type) {
    ...
    try {
      const res = await fetch(`${API_URL}/api/reports/${merchantId}/excel/${type}`, { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
  ...
  async function downloadChatgptJson() {
    if (!merchantId) return;
    try {
      const res = await fetch(`${API_URL}/api/reports/${merchantId}/chatgpt-json`, { 
        headers: { "Authorization": `Bearer ${token}` } 
      });
      ...
      <aside>
        ...
        <nav>
          {merchants.length > 0 ? merchants.map(m => (
            ...
            <div className="empty-nav">등록된 가맹점이 없습니다.</div>
          )}
        </nav>
        ...
        <div className="modal-backdrop" onClick={() => setShowMerchantModal(false)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              <button className="btn-primary" onClick={saveMerchant} disabled={savingMerchant || !merchantForm.name}>저장</button>
            </div>
          </div>
        </div>
      )}
